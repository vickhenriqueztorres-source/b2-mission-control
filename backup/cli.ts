...
async function main() {
  ...
  if (action === 'create' || args.includes('create')) {
    ...
    const manifest = await BackupManager.createBackup(backupId || undefined);
    ...
    const res = await BackupManager.verifyBackup(backupId);
    if (res.valid) {
    ...
    const res = await BackupManager.restoreBackup(backupId);
    if (res.restored) {
