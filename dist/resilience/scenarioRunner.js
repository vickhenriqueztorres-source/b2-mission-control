...
const chaosEventRecorder_1 = require("./chaosEventRecorder");
const faultInjector_1 = require("./faultInjector");
const chaosAssertionEngine_1 = require("./chaosAssertionEngine");
const cleanupManager_1 = require("./cleanupManager");
const diskSpaceProvider_1 = require("./diskSpaceProvider");
class ScenarioRunner {
    static async runScenario(scenarioId) {
        const formattedId = scenarioId.startsWith('CHAOS-') ? scenarioId : `CHAOS-${scenarioId}`;
        const contractPath = path_1.default.resolve(process.cwd(), 'resilience', 'scenarios', `${formattedId}.json`);
        if (!fs_1.default.existsSync(contractPath)) {
            throw new Error(`Contrato do cenário não encontrado: ${contractPath}`);
        }
        const rawContract = fs_1.default.readFileSync(contractPath, 'utf-8');
        ...
        const runDir = path_1.default.resolve(process.cwd(), 'runs', formattedId);
        ...
        const recorder = new chaosEventRecorder_1.ChaosEventRecorder(formattedId);
        const injector = new faultInjector_1.FaultInjector(runDir);
        ...
        const classification = contract.outcome_classification || (contract.expected_terminal_state === 'FAILED' ? 'EXPECTED_FAILURE' :
        ...
        try {
            switch (contract.injected_fault) {
                case 'schema_invalid': {
                    const invalidFile = path_1.default.join(runDir, 'invalid_motion.json');
                ...
                case 'start_frame_missing': {
                    const guideFile = path_1.default.join(runDir, 'firefly_guide.json');
                ...
                case 'incomplete_mp4': {
                    const mp4Path = path_1.default.join(runDir, 'corrupted_video.mp4');
                ...
                case 'disk_space_exhausted': {
                    ...
                    const hasSpace = diskSpaceProvider_1.DiskSpaceProvider.hasSufficientSpace(runDir);
                    ...
                    recorder.recordEvent({
                        ...
                        type: 'AGENT_FAILED',
                        status: 'FAILED',
                        message: `Espaço em disco insuficiente (Disponível: ${diskSpaceProvider_1.DiskSpaceProvider.getAvailableDiskSpaceBytes(runDir)} bytes).`,
                        attempt: 1
                    });
                    actualTerminalState = 'FAILED';
                    break;
        ...
        recorder.close();
        const stateSnapshotPath = path_1.default.join(runDir, 'state_snapshot.json');
        fs_1.default.writeFileSync(stateSnapshotPath, JSON.stringify({
        ...
        const assertionResult = chaosAssertionEngine_1.ChaosAssertionEngine.evaluateScenario(contract, events, actualTerminalState, actualRetries, runDir);
        const assertionsPath = path_1.default.join(runDir, 'assertions.json');
        fs_1.default.writeFileSync(assertionsPath, JSON.stringify({ ...assertionResult, classification }, null, 2), 'utf-8');
        cleanupManager_1.CleanupManager.cleanupStaging(runDir, contract.cleanup_strategy);
...
${assertionResult.invariants
            .map(inv => `- ${inv.passed ? '✅' : '❌'} **${inv.invariant}**: ${inv.message}`)
            .join('\n')}
