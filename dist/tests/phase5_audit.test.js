...
exports.runPhase5Audit = runPhase5Audit;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
const environment_1 = require("../config/environment");
const bootReconciler_1 = require("../orchestrator/bootReconciler");
...
async function runPhase5Audit() {
    ...
    const rebootDir = path_1.default.resolve(process.cwd(), 'runs', 'REBOOT-E2E-001');
    if (fs_1.default.existsSync(rebootDir)) {
        fs_1.default.rmSync(rebootDir, { recursive: true, force: true });
    }
    fs_1.default.mkdirSync(rebootDir, { recursive: true });
    const preBootState = {
    ...
    fs_1.default.writeFileSync(path_1.default.join(rebootDir, 'pre_boot_snapshot.json'), JSON.stringify(preBootState, null, 2), 'utf-8');
    const reconRes = bootReconciler_1.BootReconciler.reconcileBoot();
    const postBootState = {
    ...
    fs_1.default.writeFileSync(path_1.default.join(rebootDir, 'post_boot_snapshot.json'), JSON.stringify(postBootState, null, 2), 'utf-8');
    ...
    try {
        environment_1.EnvironmentConfig.assertNoChaosInProduction();
    ...
    const pauseRes = bootReconciler_1.BootReconciler.pauseNewJobs();
    const activeJobMonitoringKept = bootReconciler_1.BootReconciler.isQueuePaused === true;
    const resumeRes = bootReconciler_1.BootReconciler.resumeQueue();
    const queueResumed = bootReconciler_1.BootReconciler.isQueuePaused === false;
    ...
    const emergencyRes = bootReconciler_1.BootReconciler.emergencyStop();
    const isEmergencyActive = bootReconciler_1.BootReconciler.isEmergencyStopped === true;
    ...
    const backupManifestRes = await backupManager_1.BackupManager.createBackup('AUDIT_BKP_001');
    const verifyBackupRes = await backupManager_1.BackupManager.verifyBackup('AUDIT_BKP_001');
    const cleanRestoreDir = path_1.default.resolve(process.cwd(), 'runs', 'RESTORE_CLEAN_TEST');
    if (fs_1.default.existsSync(cleanRestoreDir)) {
        fs_1.default.rmSync(cleanRestoreDir, { recursive: true, force: true });
    }
    fs_1.default.mkdirSync(cleanRestoreDir, { recursive: true });
    const restoreRes = await backupManager_1.BackupManager.restoreBackup('AUDIT_BKP_001', cleanRestoreDir);
    ...
    const soakMetricsPath = path_1.default.resolve(process.cwd(), 'runs', 'SOAK-001', 'soak_metrics.json');
    let req6Pass = false;
    let soakTrendMsg = '';
    if (fs_1.default.existsSync(soakMetricsPath)) {
        const soakData = JSON.parse(fs_1.default.readFileSync(soakMetricsPath, 'utf-8'));
    ...
    const videoAuditEntries = [];
    const seenHashes = new Set();
    let duplicateHashes = 0;
    for (let p = 1; p <= 5; p++) {
        const prodId = `PILOT-00${p}`;
        const manifestPath = path_1.default.resolve(process.cwd(), 'runs', prodId, 'manual_kling_clip_intake.json');
    ...
    // -------------------------------------------------------------------------
    const auditReportPath = path_1.default.resolve(process.cwd(), 'PHASE-5-INDEPENDENT-AUDIT.md');
...
${videoAuditEntries
        .map((v, i) => `${i + 1}. **${v.production_id}** | ${v.shot_id} / ${v.take_id} (Job #${v.job_id}) | SHA-256: \`${v.sha256}\` | 1080p @ 30FPS | 5.0s`)
        .join('\n')}
...
`;
    fs_1.default.writeFileSync(auditReportPath, reportMd.trim(), 'utf-8');
    console.log('\n====================================================================');
