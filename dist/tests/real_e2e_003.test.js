...
async function executePhase3ThreeTakesRealAcceptanceTest() {
    ...
    const telemetry = agentTelemetryAdapter_1.AgentTelemetryAdapter.getInstance();
    const antigravity = new antigravityAdapter_1.AntigravityAdapter();
  ...
  `).run(runId);
    ...
    for (const name of takeNames) {
        const fireflySaidaFile = path_1.default.resolve(`C:/Users/brend/OneDrive/Desktop/B2 ENTERPRISE/Canais_/Mateo - Copia/agente firefly/saida/${name}.mp4`);
    ...
    for (let i = 0; i < fireflyResult.completedJobs.length; i++) {
        ...
        const targetMp4Path = path_1.default.join(runDir, `video_result_take_${i + 1}.mp4`);
        fs_1.default.copyFileSync(job.output_path, targetMp4Path);
        const buf = fs_1.default.readFileSync(targetMp4Path);
        const sha256 = crypto_1.default.createHash('sha256').update(buf).digest('hex');
        fs_1.default.writeFileSync(path_1.default.join(runDir, `video_result_take_${i + 1}.sha256`), sha256, 'utf-8');
    ...
    fireflyToIntake_1.FireflyToIntakeBridge.convert(runId, generatedVideos, intakeManifestPath, frameHash, frameHash);
