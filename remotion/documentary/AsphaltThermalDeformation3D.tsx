...
export interface AsphaltThermalDeformation3DProps {
  ...
  metric2Label?: string;
  metric2Value?: string;
  auditorCards?: Array<{ name: string; time: string; status: string; color?: string }>;
  footerNote?: string;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{display: 'flex', justifyContent: 'space-between', zIndex: 10}}>
          {metric1Label ? (
            <span style={{fontSize: 14, color: '#F4F4F0', fontWeight: 800}}>
              {metric1Label}: <strong style={{color: '#FF5500'}}>{metric1Value || `${temperatureCelsius}°C`}</strong>
            </span>
          ) : null}
          {metric2Label ? (
        ...
        {/* Auditoria de Discrepância Temporal */}
        {auditorCards && auditorCards.length > 0 ? (
          <div style={{display: 'flex', justifyContent: 'space-around', alignItems: 'center', margin: '20px 0', zIndex: 10}}>
            {auditorCards.map((l, i) => (
              <div
                key={i}
                style={{
                  backgroundColor: 'rgba(6, 7, 9, 0.85)',
                  border: `1px solid ${l.color || accentColor}`,
                  borderRadius: 6,
                  padding: '12px 16px',
                  textAlign: 'center',
                  boxShadow: `0 0 15px ${(l.color || accentColor)}30`
                }}
              >
                ...
                <div style={{fontSize: 10, color: l.color || accentColor, fontWeight: 700}}>{l.status}</div>
              </div>
            ))}
          </div>
        ) : null}
