...
export class RecoveryVerifier {
  public static verifyRecovery(
    ...
    eventsBefore: ChaosEventPayload[],
    eventsAfter: ChaosEventPayload[]
    ...
    const preservedAllPrevEvents = Array.from(initialEventIds).every(id =>
      eventsAfter.some(e => e.event_id === id)
    );
    ...
    const hasRecoveryEvent = eventsAfter.some(
      e => e.type === 'AGENT_RETRYING' || e.type === 'AGENT_COMPLETED' || e.type === 'STATE_RECOVERED'
    );
