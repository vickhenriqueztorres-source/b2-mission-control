...
scenes_to_heal = [13, 47, 48, 49, 50]

for idx in scenes_to_heal:
    ...
    dir_run = os.path.join(dst_run_ep04, ool_name)
    ...
    # Copia para os diretórios públicos e SC_XXX
    for d in [
        os.path.join(dst_public_root, sc_name),
        os.path.join(dst_public_root, ool_name),
        os.path.join(dst_public_ep04, sc_name),
        os.path.join(dst_public_ep04, ool_name),
        os.path.join(dst_run_ep04, sc_name)
    ]:
    ...
    print(f"✅ Healed {ool_name} / {sc_name}: frame ({frame_size_kb:.1f} KB), take ({take_size_kb:.1f} KB)")

print("\nCenas curadas com sucesso!")
