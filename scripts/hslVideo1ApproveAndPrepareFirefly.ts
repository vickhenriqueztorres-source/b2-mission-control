...
function main(): void {
  ...
  const manifestPath = path.join(outputRoot, 'firefly', 'video-1-firefly-preparation.json');
  writeJson(manifestPath, {
    schema: 'hsl.video-1.firefly-preparation.v1', schema_version: '1.0.0', production_id: productionId,
    status: 'FIREFLY_GUIDE_READY', paid_dispatch_authorized: false,
    ...
    firefly_guide_path: prepared.masterGuidePath, firefly_jobs: prepared.jobNames,
    prepared_at: new Date().toISOString()
  });
  process.stdout.write(`${JSON.stringify({status: 'FIREFLY_GUIDE_READY', paid_dispatch_authorized: false, approval_manifest_path: approvalManifestPath, start_frame_manifest_path: startFrames.startFrameManifestPath, motion_package_count: startFrames.motionPackagePaths.length, firefly_guide_path: prepared.masterGuidePath, preparation_manifest_path: manifestPath}, null, 2)}\n`);
}
