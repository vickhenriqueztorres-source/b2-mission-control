...
const generatedFallbackByScene: Readonly<Record<string, string>> = {
  HSL2_004: 'HSL2_006_V01',
  HSL2_013: 'HSL2_011_V01',
  HSL2_016: 'HSL2_018_V01',
  HSL2_019: 'HSL2_028_V01',
  HSL2_041: 'HSL2_040_V01'
};
...
function assertRealGeneratedAssets(
  ...
  if (usesLocalProxy(dispatchResult.status)) {
    throw new Error(`HSL_VIDEO_2_REAL_GENERATED_ASSETS_REQUIRED: dispatch status is ${dispatchResult.status}`);
  }
  if (usesLocalProxy(dispatchResult.dispatch_mode)) {
    throw new Error(`HSL_VIDEO_2_REAL_GENERATED_ASSETS_REQUIRED: dispatch mode is ${dispatchResult.dispatch_mode}`);
  }
  ...
  for (const job of completedJobs) {
    if (job && typeof job === 'object' && usesLocalProxy((job as {output_path?: unknown}).output_path)) {
      throw new Error(`HSL_VIDEO_2_REAL_GENERATED_ASSETS_REQUIRED: local proxy dispatch output for ${(job as {job_name?: unknown}).job_name || 'UNKNOWN_JOB'}`);
    }
  }
  for (const item of intake.items) {
    if (usesLocalProxy(item.video_path)) {
      throw new Error(`HSL_VIDEO_2_LOCAL_PROXY_ASSET_FORBIDDEN:${item.shot_id}:${item.video_path}`);
    }
  }
}
...
function audioDuration(filePath: string): number {
  ...
  if (result.status !== 0) throw new Error(`HSL_VIDEO_2_NARRATION_PROBE_FAILED:${result.stderr || ''}`);
...
function fitNarration(sourcePath: string, targetSeconds: number, outputPath: string): string {
  ...
  if (!Number.isFinite(tempo) || tempo < 0.5 || tempo > 2) throw new Error(`HSL_VIDEO_2_NARRATION_TEMPO_INVALID:${tempo}`);
  runFfmpeg([
    '-y', '-hide_banner', '-loglevel', 'error', '-i', sourcePath,
    '-filter:a', `atempo=${tempo.toFixed(6)}`, '-c:a', 'libmp3lame', '-b:a', '192k', outputPath
  ], 'HSL_VIDEO_2_NARRATION_FIT_FAILED');
  return outputPath;
}
...
async function main(): Promise<void> {
  ...
  const intakePath = path.join(runRoot, 'hsl_video_2_asset_intake.json');
  ...
  if (!fs.existsSync(finalNarrationPath)) {
    throw new Error('HSL_VIDEO_2_OFFICIAL_NARRATION_REQUIRED: run hsl:video-2-narration with the official Echo/Voicebox voice before finishing');
  }
  for (const required of [sourcePlanPath, intakePath, dispatchResultPath, narrationSourcePath]) {
    if (!fs.existsSync(required)) throw new Error(`HSL_VIDEO_2_FINISH_INPUT_REQUIRED:${required}`);
  }
  ...
  for (const relative of sourcePlan.scenes) {
    ...
    if (fallbackSourceShotId) {
      ...
      if (!sourceAsset) throw new Error(`HSL_VIDEO_2_FALLBACK_ASSET_REQUIRED:${fallbackSourceShotId}`);
      ...
      scene.visual_shots = (scene.visual_shots || []).map((shot) => {
        ...
        return {
          ...
          visual_function: 'reconstruction', generation_strategy: 'VEO_MOTION_GRAPHIC', audio_strategy: 'VEO_NATIVE'
        };
      });
    }
  ...
  const combinedIntakePath = path.join(runRoot, 'hsl_video_2_postproduction_intake.json');
  ...
  writeJson(fallbackManifestPath, {
    schema: 'hsl.video-2.licensed-footage-fallback.v1', status: 'GENERATED_AI_FALLBACK_APPLIED',
  ...
  ], 'HSL_VIDEO_2_CONTACT_SHEET_FAILED');
  const renderManifest = JSON.parse(fs.readFileSync(postproduction.renderManifestPath, 'utf8')) as Record<string, unknown>;
  const finalManifestPath = path.join(runRoot, 'video-2-final-manifest.json');
  writeJson(finalManifestPath, {
    schema: 'hsl.video-2.final-manifest.v1', schema_version: '1.0.0', status: 'HSL_VIDEO_2_COMPLETE',
    production_id: productionId, title: 'What Happens When Your Bag Misses Its Connection?',
  ...
  process.stdout.write(`${JSON.stringify({
    status: 'HSL_VIDEO_2_COMPLETE', title: 'What Happens When Your Bag Misses Its Connection?',
    narration_status: narrationProvider === 'voicebox' ? 'VOICEBOX_LOCAL_FINAL' : 'ELEVENLABS_FINAL',
