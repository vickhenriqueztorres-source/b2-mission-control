...
function main(): void {
  ...
  const expected = new Map(scenes.flatMap((scene) => scene.visual_shots)
    ...
    .map((shot) => [shot.shot_id, shot.generation_strategy?.startsWith('VEO') ? 'Veo 3.1 Fast' : 'Kling 3.0'] as const));
  ...
  for (const item of intake.items) {
    ...
    if (item.model === 'Veo 3.1 Fast' && item.visual_qa.first_frame_fidelity !== 'FIRST_FRAME_FIDELITY_PASS') {
  ...
  writeJson(qaPath, {
    schema: 'hsl.video-2.generated-video-qa.v1', production_id: productionId, status,
    item_count: intake.items.length, model_counts: {'Veo 3.1 Fast': veoItems.length, 'Kling 3.0': klingItems.length},
