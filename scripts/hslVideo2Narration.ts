...
async function main(): Promise<void> {
  ...
  if (fs.existsSync(outputPath) && fs.statSync(outputPath).size > 0) {
    process.stdout.write(`${JSON.stringify({
      status: 'NARRATION_ALREADY_PRESENT', official_voice: process.env.HSL_OFFICIAL_VOICE_NAME || 'Echo',
      provider: process.env.HSL_NARRATION_PROVIDER || 'voicebox',
      profile_id: process.env.HSL_VOICEBOX_PROFILE_ID || null,
      preset_voice_id: process.env.HSL_VOICEBOX_PRESET_VOICE_ID || null,
      scene_count: scenes.length, output_path: outputPath
    }, null, 2)}\n`);
    return;
  }
  await new NarrationVoiceAgent().generate(text, outputPath);
  process.stdout.write(`${JSON.stringify({
    status: 'NARRATION_GENERATED', official_voice: process.env.HSL_OFFICIAL_VOICE_NAME || 'Echo',
    provider: process.env.HSL_NARRATION_PROVIDER || 'voicebox',
    profile_id: process.env.HSL_VOICEBOX_PROFILE_ID || null,
    preset_voice_id: process.env.HSL_VOICEBOX_PRESET_VOICE_ID || null,
    scene_count: scenes.length, character_count: text.length,
    output_path: outputPath, byte_count: fs.statSync(outputPath).size
  }, null, 2)}\n`);
}
