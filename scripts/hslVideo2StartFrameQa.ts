...
function main(): void {
  ...
  const contactSheetPath = contactSheetPaths[0];
  ...
  writeJson(manifestPath, {
    ...
    contact_sheet_paths: contactSheetPaths, contact_sheet_page_size: 24, items
  });
  process.stdout.write(`${JSON.stringify({status: 'START_FRAME_SET_QA_PASS', item_count: items.length, qa_manifest_path: manifestPath, contact_sheet_paths: contactSheetPaths, human_review_status: 'PENDING'}, null, 2)}\n`);
}
