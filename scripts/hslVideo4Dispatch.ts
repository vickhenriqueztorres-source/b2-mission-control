...
function forceFireflyVideo720p(
  ...
  for (const item of guide.items || []) {
    ...
    if (!lineage) throw new Error(`HSL_VIDEO_4_LINEAGE_MISSING:${jobName}`);
...
async function main(): Promise<void> {
  if (process.env.HSL_CONFIRMED_VIDEO_4_DISPATCH !== 'true' || process.env.HSL_ALLOW_PAID_FIREFLY_DISPATCH !== 'true') {
    throw new Error('HSL_VIDEO_4_DISPATCH_NOT_AUTHORIZED');
  }
  ...
  if (expectedShotIds.size > 60) throw new Error(`HSL_VIDEO_4_GENERATED_JOB_GUARDRAIL:${expectedShotIds.size}`);
  const preparationPath = path.join(outputRoot, 'firefly', 'video-4-generation-preparation.json');
  ...
  if (preparation.status !== 'GENERATION_GUIDE_READY' || preparation.motion_package_count !== expectedShotIds.size) {
    throw new Error('HSL_VIDEO_4_GENERATION_PREPARATION_INVALID');
  }
  ...
  const reroutePath = path.join(outputRoot, 'firefly', 'video-4-provider-reroutes.json');
  writeJson(reroutePath, {schema: 'hsl.video-4.provider-reroutes.v1', status: 'ACTIVE', items: providerReroutes});
  const adapter = new FireflyAdapter();
  await adapter.initialize();
  const generated = await runtime.dispatch(productionId, prepared, adapter);
  const intakeManifestPath = path.join(outputRoot, 'hsl_video_4_asset_intake.json');
