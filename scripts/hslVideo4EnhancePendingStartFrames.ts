...
function renderPng(buffer: Buffer, outputPath: string, width = WIDTH, height = HEIGHT): void {
  ...
  if (result.status !== 0) throw new Error(`HSL_VIDEO_4_PREMIUM_FRAME_RENDER_FAILED:${result.stderr || result.stdout}`);
}
...
function main(): void {
  ...
  const pendingJobs = db.prepare("select id,name,image_path,status from jobs where name like 'HSL4_%' and status = 'pending' order by id").all() as JobRow[];
  ...
  const enhanced = pendingJobs.map((job, index) => {
    ...
    if (!item) throw new Error(`HSL_VIDEO_4_PENDING_SHOT_NOT_IN_PLAN:${shotId}`);
    if (!job.image_path) throw new Error(`HSL_VIDEO_4_PENDING_JOB_WITHOUT_IMAGE:${job.name}`);
  ...
  writeJson(manifestPath, {
    schema: 'hsl.video-4.premium-pending-start-frame-enhancement.v1',
