...
function assertRealGeneratedAssets(intake: HslGeneratedAssetIntakeManifest, dispatchResult: Record<string, unknown>): void {
  if (usesLocalProxy(dispatchResult.status)) throw new Error(`HSL_VIDEO_4_REAL_GENERATED_ASSETS_REQUIRED:${dispatchResult.status}`);
  if (usesLocalProxy(dispatchResult.dispatch_mode)) throw new Error(`HSL_VIDEO_4_REAL_GENERATED_ASSETS_REQUIRED:${dispatchResult.dispatch_mode}`);
  ...
  for (const job of completedJobs) {
    if (job && typeof job === 'object' && usesLocalProxy((job as {output_path?: unknown}).output_path)) {
      throw new Error(`HSL_VIDEO_4_LOCAL_PROXY_DISPATCH_OUTPUT:${(job as {job_name?: unknown}).job_name || 'UNKNOWN_JOB'}`);
    }
  }
  for (const item of intake.items) {
    if (usesLocalProxy(item.video_path)) throw new Error(`HSL_VIDEO_4_LOCAL_PROXY_ASSET_FORBIDDEN:${item.shot_id}:${item.video_path}`);
    if (item.model !== 'Firefly Video') throw new Error(`HSL_VIDEO_4_FIREFLY_VIDEO_REQUIRED:${item.shot_id}:${item.model}`);
  }
}
...
function audioDuration(filePath: string): number {
  ...
  if (result.status !== 0) throw new Error(`HSL_VIDEO_4_NARRATION_PROBE_FAILED:${result.stderr || ''}`);
...
function fitNarration(sourcePath: string, targetSeconds: number, outputPath: string): string {
  ...
  if (!Number.isFinite(tempo) || tempo < 0.5 || tempo > 2) throw new Error(`HSL_VIDEO_4_NARRATION_TEMPO_INVALID:${tempo}`);
  runFfmpeg([
    ...
    '-b:a', '192k',
    outputPath
  ], 'HSL_VIDEO_4_NARRATION_FIT_FAILED');
  return outputPath;
}
...
async function main(): Promise<void> {
  ...
  for (const required of [sourcePlanPath, intakePath, dispatchResultPath, narrationSourcePath]) {
    if (!fs.existsSync(required)) throw new Error(`HSL_VIDEO_4_FINISH_INPUT_REQUIRED:${required}`);
  }
  ...
  const combinedIntakePath = path.join(runRoot, 'hsl_video_4_postproduction_intake.json');
  ...
  runFfmpeg([
    ...
    '-frames:v', '1',
    contactSheetPath
  ], 'HSL_VIDEO_4_CONTACT_SHEET_FAILED');
