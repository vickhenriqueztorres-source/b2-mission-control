...
function main(): void {
  ...
  if (!fs.existsSync(handoffPath)) throw new Error(`HSL_VIDEO_5_HANDOFFS_REQUIRED:${handoffPath}`);

  const db = new Database(dbPath, {readonly: true});
  const rows = db.prepare("select id,name,output_path,status from jobs where name like 'HSL5_%' and status = 'done' order by id").all() as JobRow[];
  ...
  if (rows.length !== handoffSet.handoffs.length) {
    throw new Error(`HSL_VIDEO_5_DONE_JOB_COUNT_MISMATCH:${rows.length}:EXPECTED:${handoffSet.handoffs.length}`);
  }
  ...
  const completedJobs = rows.map((row) => {
    if (!row.output_path || !fs.existsSync(row.output_path)) throw new Error(`HSL_VIDEO_5_DONE_OUTPUT_MISSING:${row.name}`);
  ...
  const items = rows.map((row): HslGeneratedAssetIntakeItem => {
    ...
    if (!lineage) throw new Error(`HSL_VIDEO_5_LINEAGE_MISSING:${shotId}`);
    if (!row.output_path) throw new Error(`HSL_VIDEO_5_DONE_OUTPUT_MISSING:${row.name}`);
    const validation = validateVideoWithFfprobe(row.output_path);
    if (!validation.valid) throw new Error(`HSL_VIDEO_5_MEDIA_INVALID:${row.name}:${validation.ffprobe_stderr}`);
  ...
  writeJson(referenceQaPath, {
    schema: 'hsl.video-5.firefly-video-reference-frame-qa.v1',
    production_id: productionId,
  ...
  process.stdout.write(`${JSON.stringify({
    status: 'HSL_VIDEO_5_INTAKE_RECOVERED',
