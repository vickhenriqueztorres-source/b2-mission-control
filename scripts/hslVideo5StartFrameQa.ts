...
import {sha256File, sha256Text, StartFrameIdentityGate} from '../hsl/startframe/startFrameIdentityGate';
...
function main(): void {
  ...
  if (duplicateHashCount > 0) throw new Error(`HSL_VIDEO_5_DUPLICATE_START_FRAME_HASH:${duplicateHashCount}`);
  ...
  new StartFrameIdentityGate().validate({
    ...
    expectedShots: items.map((item) => ({
      shot_id: item.shot_id,
      frame_path: item.candidate_path,
      start_frame_prompt: item.start_frame_prompt || ''
    }))
  });
  const contactSheetPaths = renderContactSheets(items, candidatesRoot);
  const contactSheetHashes = contactSheetPaths.map(sha256File);
  const reviewArtifactSha256 = sha256Text(contactSheetHashes.join('|'));
  const manifestPath = path.join(candidatesRoot, 'start-frame-technical-qa.json');
  writeJson(manifestPath, {
    schema: 'hsl.video-5.start-frame.technical-qa.v1',
    schema_version: '1.0.0',
    episode_id: plan.episode_id,
    production_id: productionId,
    status: 'START_FRAME_SET_QA_PASS',
    visual_identity_contract_version: HSL_VISUAL_IDENTITY_CONTRACT_VERSION,
    identity_gate_status: 'HSL_VISUAL_IDENTITY_QA_PASS',
    provenance_manifest_path: provenancePath,
    provenance_manifest_sha256: sha256File(provenancePath),
    ...
    contact_sheet_sha256: contactSheetHashes,
