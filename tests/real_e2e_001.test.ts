...
async function executeRealVerticalIntegrationTest() {
  ...
  const eventsLogFile = path.join(runDir, 'events.jsonl');
  const eventsLogStream = fs.createWriteStream(eventsLogFile, { flags: 'w' });

  const stateTransitions: Array<{ timestamp: string; old_status: string; new_status: string; metadata?: any }> = [];
  ...
  const fireflySaidaFile = path.resolve(`C:/Users/brend/OneDrive/Desktop/B2 ENTERPRISE/Canais_/Mateo - Copia/agente firefly/saida/${shotTakeName}.mp4`);
  ...
  if (!completedVideo || !fs.existsSync(completedVideo.output_path)) {
    throw new Error('Geração do Firefly falhou em produzir o arquivo MP4 final.');
  }

  const realVideoPath = completedVideo.output_path;
  const targetMp4Path = path.join(runDir, 'video_result.mp4');
  fs.copyFileSync(realVideoPath, targetMp4Path);

  const videoBuffer = fs.readFileSync(targetMp4Path);
  const videoHash = crypto.createHash('sha256').update(videoBuffer).digest('hex');
  fs.writeFileSync(path.join(runDir, 'video_result.sha256'), videoHash, 'utf-8');
  ...
  FireflyToIntakeBridge.convert(runId, [{ name: shotTakeName, output_path: targetMp4Path }], intakeManifestPath, frameHash, frameHash);
  ...
  fs.writeFileSync(path.join(runDir, 'qa_result.json'), JSON.stringify(qaResult, null, 2), 'utf-8');
  ...
  fs.writeFileSync(path.join(runDir, 'state_transitions.json'), JSON.stringify(stateTransitions, null, 2), 'utf-8');
